...
public class ChartDataManager3 {
    ...
    public ChartDataManager3(Context context, String deviceId) {
        mContext = context;
        mDeviceId = deviceId;
    }
...
//                Log.e(TAG, "getIAQDataHistory: responseStr=" + responseStr);
    ...
    private void addHourZeroPoint() {
        ...
        Log.e("---addHourZeroPoint---",""+hourArrayAir.size());
        mIaqHistoryDataList = setHourEmotionData(hourArrayAir);
        Log.e("---mIaqHisto---",""+mIaqHistoryDataList.size());
        for (int i = 0;i< mIaqHistoryDataList.size();i++){
            Log.e("---item---",""+mIaqHistoryDataList.get(i).getTimestamp());
        }
    }

    private void addDayZeroPoint() {
        List<String> dateArrayAir = getDateStringsAir();
        Log.e("---dateArrayAir---",""+dateArrayAir.size());
        for (int i = 0;i< dateArrayAir.size();i++){
            Log.e("---item---",""+dateArrayAir.get(i));
        }
    ...
    private List<String> getDateStringsAir() {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            Log.e("当天数据起始点时间", "" + fromDateString);
